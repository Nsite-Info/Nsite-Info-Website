...
const selectUser = async (user) => {
  try {
    ...
    if (hex && hex !== "Not found") {
      ...
      window.location.href = "https://"+ convertHexToNpub(user.npub) + ".nsite.lol";
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (hex !== "Not found") {
        ...
        window.location.href = `https://${npub}.nsite.lol`;
...
const resolve = () => {
  ...
  if (searchTerm.value == '') {
    ...
    try {
      window.location.href = `https://${searchTerm.value}.nsite.lol`;
    } catch (error) {
      console.error("Failed to fetch user profile:", error);
    }
